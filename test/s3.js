...
function sign(secret, httpVerb, bucket, path, params) {
    ...
    return require('crypto').createHmac('sha1', secret).update(stringToSign).digest('base64')
}

function restCall(id, secret, httpVerb, bucket, path, data, params, encoding) {
    ...
    if (data) {
        ...
        params["Content-MD5"] = require('crypto').createHash('md5').update(data, buffer ? null : 'utf8').digest('base64')
    }
    ...
    var r = _.p()
    var ret = _.consume(r, encoding)
    if (Math.floor(r.statusCode / 100) == 2) return ret
    throw new Error(r.statusCode + " : " + ret)
}

module.exports = function (id, secret, bucket) {
    var self = {}
    ...
    self.restCall = function (httpVerb, path, data, params, encoding) {
        return restCall(id, secret, httpVerb, bucket, path, data, params, encoding)
    }

    self.del = function (path) {
        return self.restCall('delete', path)
    }

    self.get = function (path, encoding) {
        return self.restCall('get', path, null, null, encoding)
    }

    self.put = function (path, data, isPublic) {
        var params = null
        if (isPublic) {
            params = {
        ...
        try {
            self.restCall('put', path, data, params)
        } catch (e) {
            self.restCall('put')
            self.restCall('put', path, data, params)
        }
        return 'http://' + bucket + '.s3.amazonaws.com' + prependSlash(path)
    }
